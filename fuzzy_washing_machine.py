...
def get_rule_antecedent_value(ant1: FuzzySet, val1: float, ant2: FuzzySet, val2: float, operator: str) -> float:
    if ant1 is None:
        degree_2 = fuzzify(ant2, val2)
        return degree_2

    if ant2 is None:
        degree_1 = fuzzify(ant1, val1)
        return degree_1
        ...
        degree_2 = fuzzify(ant2, val2)
        if operator == "OR":
            return get_disjunction(degree_1, degree_2)
        elif operator == "AND":
            return get_conjunction(degree_1, degree_2)
...
def configure_washing_machine(dirt_amount: float, fabric_weight: float) -> tuple:
    ...
    # a1 = get_rule_antecedent_value(None, 0.0, very_delicate_set, fabric_weight, "''")
    # a2 = get_rule_antecedent_value(almost_clean_set, dirt_amount, delicate_set, fabric_weight, "OR")
    # a3 = get_rule_antecedent_value(dirty_set, dirt_amount, delicate_set, fabric_weight, "AND")
    # a4 = get_rule_antecedent_value(dirty_set, dirt_amount, not_delicate_set, fabric_weight, "AND")
    all_antecedents.append(get_rule_antecedent_value(None, 0.0, very_delicate_set, fabric_weight, "''"))
    all_antecedents.append(get_rule_antecedent_value(almost_clean_set, dirt_amount, delicate_set, fabric_weight, "OR"))
    all_antecedents.append(get_rule_antecedent_value(dirty_set, dirt_amount, delicate_set, fabric_weight, "AND"))
    all_antecedents.append(get_rule_antecedent_value(dirty_set, dirt_amount, not_delicate_set, fabric_weight, "AND"))
    all_outputs.append(get_rule_output_value(1, all_antecedents[0]))
    all_outputs.append(get_rule_output_value(2, all_antecedents[1]))
    all_outputs.append(get_rule_output_value(3, all_antecedents[2]))
    all_outputs.append(get_rule_output_value(4, all_antecedents[3]))
...
def get_temperature(all_antecedents: List, all_outputs: List) -> float:
    return 10 + get_weighted_average(all_antecedents, all_outputs) * 0.8
